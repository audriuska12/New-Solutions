...
<html>
    ...
    <body>
        ...
        if ($count > 0) {
            echo ("<table>");
            echo ("<tr><th>Pavadinimas</th><th>Stažas</th><th>Profesionalumo lygis</th><th>Sveikatos sutrikimai</th>" . (($user->arVirsesnis($darbuotojas) || ($user->id==$darbuotojas->id && $user->getRusis()->pavadinimas == "Parduotuvių tinklo vadovas")) ? "<th></th>" : "") . "</tr>");
            for ($i = 0; $i < $count; $i++) {
                echo ("<tr><td>" . $pareigos[$i]->pavadinimas . "</td><td>" . $pareigos[$i]->stazas . "</td><td>" . $pareigos[$i]->profesionalumo_lygis . "</td><td>" . $pareigos[$i]->sveikatos_sutrikimai . "</td>" . (($user->arVirsesnis($darbuotojas) || ($user->id==$darbuotojas->id && $user->getRusis()->pavadinimas == "Parduotuvių tinklo vadovas")) ? "<td><a href=\"atimtiPareigas.php?pareigos=" . $pareigos[$i]->id . "&darbuotojas=" . $_GET['id'] . "\" onclick=\"return confirm('Ar tikrai norite atimti šias pareigas?')\">Atimti</a></td>" : "") . "</tr>");
            }
            echo("</table>");
        } else {
