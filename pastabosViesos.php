...
$pastabos=$darbuotojas->getGautosPastabosViesos();
$count=count($pastabos);
if($count>0){
    echo("<table>");
    echo("<tr><th>Rašė</th><th>Data</th><th>Tekstas</th></tr>");
    for($i=0; $i<$count; $i++){
        $rasytojas=darbuotojas::getFromDatabase($pastabos[$i]->rasytojas);
        echo("<tr><td>".$rasytojas->pavarde." ".$rasytojas->vardas."</td><td>".$pastabos[$i]->rasymo_data."</td><td>".$pastabos[$i]->tekstas."</td></tr>");
    }
    echo("</table>");
}
